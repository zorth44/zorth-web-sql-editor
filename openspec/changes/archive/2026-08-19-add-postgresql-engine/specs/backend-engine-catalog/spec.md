...
- **THEN** the service SHALL return `{ items }` containing every registered engine in registry order, currently `MYSQL` then `POSTGRESQL`, and SHALL NOT require `DATA_SOURCE_MANAGE`
...
- **WHEN** MYSQL or POSTGRESQL lists the default database field
