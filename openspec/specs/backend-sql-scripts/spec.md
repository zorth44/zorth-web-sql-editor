# Backend SQL Scripts Specification

## Purpose

Define current-user private SQL script persistence in the metadata database: isolation, validation, quota, optimistic locking, and CRUD contracts. Script bodies are stored as SQL text in the metadata schema, not on the application host filesystem or in object storage.
