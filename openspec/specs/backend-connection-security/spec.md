...
The SQL service SHALL build MySQL JDBC URLs exclusively from structured validated fields and SHALL apply user properties before non-overridable security properties.

#### Scenario: Accept an allowed property
- **WHEN** `properties` contains `serverTimezone`, `characterSetResults`, `zeroDateTimeBehavior`, `tinyInt1isBit`, or `sendFractionalSeconds` with an allowed value
- **THEN** the service SHALL include it in the generated JDBC configuration
...
#### Scenario: Enforce fixed security properties
- **WHEN** a JDBC configuration is built
...
- **WHEN** SSL mode is `DISABLED`, `PREFERRED`, or `REQUIRED`
- **THEN** the service SHALL apply the documented Connector/J flags and SHALL NOT claim CA or hostname verification
