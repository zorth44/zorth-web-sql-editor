...
The application SHALL expose the reviewed phase-one routes without delivering phase-two functionality.
...
- **THEN** the router SHALL resolve the corresponding phase-one page with the documented authentication requirement

#### Scenario: Navigate to the phase-two editor
- **WHEN** a user visits `/sql-editor` during phase one
- **THEN** the application SHALL redirect to `/data-sources` or show an explicit not-yet-available state without loading editor code
...
Protected phase-one pages SHALL use a consistent desktop-oriented application shell.

#### Scenario: Render the shell
- **WHEN** Session validation succeeds
- **THEN** the top navigation SHALL display the application identity, current product name, and a user menu containing logout
...
The frontend SHALL provide repeatable static, unit/component, end-to-end, and production-build verification.

#### Scenario: Verify a candidate implementation
- **WHEN** the project's verification commands run
- **THEN** formatting/lint, type checking, Vitest, Playwright, and Vite production build SHALL complete through documented pnpm scripts
