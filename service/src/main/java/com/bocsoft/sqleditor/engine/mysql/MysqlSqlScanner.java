...
final class MysqlSqlScanner {
    ...
    List<String> split(String sql) {
        ...
        for (int i = 0; i < sql.length(); i++) {
            ...
            switch (state) {
                case NORMAL:
                    ...
                    else if (c == '#') state = State.LINE;
                    else if (c == '-' && n == '-' && (i + 2 >= sql.length() || Character.isWhitespace(sql.charAt(i + 2)))) { state = State.LINE; i++; }
        ...
        if (state == State.SINGLE || state == State.DOUBLE || state == State.BACKTICK || state == State.BLOCK) {
    ...
    private enum State { NORMAL, SINGLE, DOUBLE, BACKTICK, LINE, BLOCK }
}
